...
public:
	...
	void printAllSurName_Name_MName_bYaear_Marks543() {
		StringBuilderClass* sb = new StringBuilderClass();
		for (auto item : DataBase) {
			cout << item.surName + " " + item.name + " " + item.middleName + " " + sb->split(item.birthDateString, '.', 2) + " " + std::to_string(item.countMarks5) + " " + std::to_string(item.countMarks4) + " " + std::to_string(item.countMarks3)+ " " + std::to_string(item.zachet) << endl;
		}
		delete sb;
	}
